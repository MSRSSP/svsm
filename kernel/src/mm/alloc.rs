...
impl MemoryRegion {
    ...
    fn merge_pages(&mut self, pfn1: usize, pfn2: usize, order: usize) -> Result<usize, AllocError> {
        ...
        proof! {
            ...
            self.lemma_accounting((order + 1) as usize);
        ...
        proof! {
            vstd::set_lib::lemma_int_range(pfn as int, pfn + (1usize << (order + 1)));
            old(self)@.reserved().lemma_pfn_dom_update(self@.reserved(), set_int_range(pfn as int, pfn + (1usize << (order + 1))), order, (order + 1) as usize);
            old(self).lemma_nr_page_add(self, -2, order);
            old(self).lemma_nr_page_add(self, 1, (order + 1) as usize);
            assert forall|o, i| 0 <= o < MAX_ORDER && 0 <= i < self@.next[o].len()
                implies self@.wf_at(o, i) by {
                    if i < old(self)@.next[o].len() {
                        assert(old(self)@.wf_at(o, i));
