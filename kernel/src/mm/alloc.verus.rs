    ...
    spec fn wf_ptr(&self, map: LinearMap) -> bool {
        forall|pfn: usize|
            0 <= pfn < self.page_count() ==> #[trigger] self.reserved[pfn as int].addr()
                == spec_map_page_info_addr(map, pfn).spec_int_addr().unwrap()
    }
...
impl MemoryRegion {
    ...
    spec fn ens_get_next_page(
        ...
        &&& ret.is_ok() ==> {
            &&& new_tmp.last().wf_pfn_order(new_self@.map, ret.unwrap(), order as usize)
            &&& new_tmp === tmp.push(
                new_tmp.last(),
            )
    ...
    spec fn ens_try_to_merge_page_ok(
        ...
        &&& new.wf()
        &&& self.with_same_mapping(new)
        &&& new.tmp_perms@@.last().wf_pfn_order(new@.map, new_pfn, new_order)
        &&& new.tmp_perms@@.len() >= 1
