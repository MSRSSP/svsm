...
#[inline]
#[verus_verify]
#[verus_spec(ret => ensures sign_extend_ensures(addr, ret))]
    ...
    Copy + From<InnerAddr> + Into<InnerAddr> + PartialEq + Eq + PartialOrd + Ord
{
    ...
    #[inline]
    #[verus_spec(returns from_spec::<_, InnerAddr>(*self))]
    fn bits(&self) -> InnerAddr {
        (*self).into()
    }

    #[inline]
    #[verus_spec(ret => ensures ret == (from_spec(*self) === 0usize))]
    ...
    #[inline]
    #[verus_verify(external_body)] // verus does not support call from trait default method to same trait.
    #[verus_spec(
        requires
    ...
    #[inline]
    #[verus_spec(returns addr_is_page_aligned_spec(*self))]
    ...
    #[inline]
    #[verus_spec(ret => ensures ret == pfn_spec(from_spec(*self)))]
...
impl From<InnerAddr> for PhysAddr {
    #[verus_spec(returns PhysAddr::from_spec(addr))]
    #[inline]
...
impl From<u64> for PhysAddr {
    ...
    #[verus_spec(returns PhysAddr::from_spec(addr as usize))]
    #[inline]
...
impl VirtAddr {
    ...
    #[verus_spec(ret =>
        ensures ret@ == 0,
        returns VirtAddr::from_spec(0usize),
    )]
    pub const fn null() -> Self {
        Self(0)
    }
    ...
    #[inline]
    #[verus_verify]
    #[verus_spec(ret => ensures ret.new_ensures(addr))]
    ...
    #[verus_spec(ret =>
        requires L <= 5,
        ensures self.pgtbl_idx_ensures(L, ret)
    )]
    ...
    // To a pointer used by permission-tracked mem access.
    #[inline]
    #[verus_spec(ret => ensures ret.addr() == self@)]
    pub fn as_pptr<T>(&self) -> vstd::simple_pptr::PPtr<T> {
        vstd::simple_pptr::PPtr::from_usize(self.0)
    }
    ...
    #[verus_spec(ret =>
        requires Self::spec_add_requires(*self, offset),
        ensures Self::spec_add_ensures(*self, offset, ret),
    )]
...
impl From<VirtAddr> for InnerAddr {
    #[inline]
    #[verus_spec(ret => ensures addr@ == ret)]
...
impl From<u64> for VirtAddr {
    #[inline]
    #[verus_verify(external_body)]
    #[verus_spec(ret => ensures ret.new_ensures(addr as usize))]
...
impl From<VirtAddr> for u64 {
    #[inline]
    #[verus_spec(ret => ensures ret == addr@)]
    fn from(addr: VirtAddr) -> Self {
        addr.0 as u64
    }
}
