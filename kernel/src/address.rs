    ...
    Copy + From<InnerAddr> + Into<InnerAddr> + PartialEq + Eq + PartialOrd + Ord
{
    ...
    #[inline]
    #[verus_verify]
    #[verus_spec(ret => ensures ret === from_spec(*self))]
    ...
    #[verus_spec(ret =>
        requires
            align_up_requires(from_spec(*self), align),
        ensures
            ret === addr_align_up(*self, align)
    )]
    ...
    #[verus_spec(ret =>
        requires
            align_up_requires(from_spec(*self), PAGE_SIZE),
        ensures
            ret === addr_page_align_up(*self),
    )]
    ...
    #[verus_spec(ret =>
        ...
        ensures
            ret == addr_is_aligned_spec(*self, align),
    )]
    ...
    #[verus_spec(ret =>
        ...
        ensures
            ret == addr_is_aligned_spec(*self, core::mem::align_of::<T>()),
    )]
    ...
    #[inline]
    #[verus_verify]
    #[verus_spec(ret => ensures ret == addr_is_page_aligned_spec(*self))]
    ...
    #[verus_spec(ret =>
        ...
        ensures
            ret == crosses_page_spec(*self, size),
    )]
...
impl VirtAddr {
    ...
    #[verus_spec(ret =>
        ...
        ensures self.const_add_ensures(offset, ret),
    )]
...
impl ops::Sub<VirtAddr> for VirtAddr {
    ...
    #[verus_spec(ret =>
        ...
        ensures self.sub_ensures(other, ret),
    )]
...
impl ops::Sub<usize> for VirtAddr {
    ...
    #[verus_spec(ret =>
        ...
        ensures self.sub_usize_ensures(other, ret),
    )]
...
impl ops::Add<InnerAddr> for VirtAddr {
    ...
    #[verus_spec(ret =>
        ...
        ensures self.const_add_ensures(other, ret),
    )]
    fn add(self, other: InnerAddr) -> Self {
        proof! {use_type_invariant(self);}
        VirtAddr::from(self.0 + other)
