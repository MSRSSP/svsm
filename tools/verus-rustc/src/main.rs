...
fn main() -> std::io::Result<()> {
    ...
    let main_target = std::env::var("CARGO_PKG_NAME").unwrap_or_default();
    ...
    let verus_target_path = PathBuf::from(env::var("VERUS_TARGET_PATH").unwrap_or_default());
    let verifier_path = verus_target_path.join("verus");
